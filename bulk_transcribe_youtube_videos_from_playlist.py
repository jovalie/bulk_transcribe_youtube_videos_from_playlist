...
def add_to_system_path(new_path):
    ...
    if (
        ...
        os.environ["PATH"] = (
            f'"{new_path}"' + os.pathsep + os.environ["PATH"].replace(new_path, "")
        )
...
def remove_unwanted_segments_from_json(
    json_file_path, unwanted_text="Subtitles by the Amara.org community"
):
...
async def compute_transcript_with_whisper_from_audio_func(
    audio_file_path, audio_file_name, audio_file_size_mb
):
    cuda_toolkit_path = get_cuda_toolkit_path()
    if cuda_toolkit_path:
        add_to_system_path(cuda_toolkit_path)
    ...
    if use_openai_api_for_transcription:
        ...
        audio_duration_seconds = await get_audio_duration(audio_file_path)
        with tqdm(
            total=audio_duration_seconds,
            desc=f"Transcribing {audio_file_name}",
            unit="s",
        ) as pbar:
            segments, info = await asyncio.to_thread(
                model.transcribe, audio_file_path, beam_size=10, vad_filter=True
            )
            for segment in segments:
                pbar.update(segment.end - segment.start)
                print(
                    f"Processing segment: [Start: {segment.start:.2f}s, End: {segment.end:.2f}s] for file {audio_file_name} with text: {segment.text}"
                )
    ...
    with open(
        f"generated_transcript_combined_texts/{audio_file_name}.txt", "w"
    ) as file:
...
async def process_video_or_playlist(
    url, max_simultaneous_downloads, max_workers_transcribe
):
    if convert_single_video:
        yt = YouTube(url)
        videos = [yt]
            ...
            async with download_semaphore:
                ...
                if audio_path and audio_filename:
                    ...
                    await compute_transcript_with_whisper_from_audio_func(
                        audio_path, audio_filename, audio_file_size_mb
                    )
        except Exception as e:
            traceback.print_exc()
...
if __name__ == "__main__":
    ...
    asyncio.run(
        process_video_or_playlist(
            url_to_process, max_simultaneous_youtube_downloads, max_workers_transcribe
        )
    )
